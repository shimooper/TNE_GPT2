...
from evaluator_utils.mcf1_measure import MCF1Measure
...
preposition_dict = {prep: index for index, prep in enumerate(preposition_list)}
...
def main():
    ...
    parse.add_argument("--predictions_file", type=str, help="predictions file", default="train")
    parse.add_argument("--gold_file", type=str, help="gold labels file", default="train")
    parse.add_argument("--output_file", type=str, help="output file where the metrics will be written", default="train")

    args = parse.parse_args()

    print('=== Reading answers and predictions files ===')

    document_answers = load_data(args.gold_file)
    document_preds = load_data(args.predictions_file)

    print('=== Evaluating... ===')
    results = evaluate_documents(document_answers, document_preds)

    print('=== Writing results to file ===')
    to_file(results, args.output_file)
